...
public:
	PatchHook(const short* signature, unsigned int signatureLen, int offset, const void* replacement, std::size_t replaceLen);
	PatchHook(std::uintptr_t address, int offset, const void* replacement, std::size_t replaceLen);
	~PatchHook();
	bool install();
};